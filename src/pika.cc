...
int main(int argc, char *argv[]) {
    ...
    char c;
    char path[1024];
    while (-1 != (c = getopt(argc, argv, "c:hv"))) {//getopt是c语言的一个函数 解析命令行参数
        switch (c) {
            case 'c':
    ...
    if (getrlimit(RLIMIT_NOFILE, &limit) == -1) {
        ...
        if (setrlimit(RLIMIT_NOFILE, &limit) != -1) {
            LOG(WARNING) << "your 'limit -n ' of " << old_limit
                         << " is not enough for Redis to start. pika have successfully reconfig it to "
                         << limit.rlim_cur;
        } else {
